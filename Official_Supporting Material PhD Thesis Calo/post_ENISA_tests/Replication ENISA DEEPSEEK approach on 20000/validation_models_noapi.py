...
def query_model(prompt, client, retries=1):
    for attempt in range(retries + 1):
        try:
            print(f"\n🚀 Sending prompt (length={len(prompt)} chars)...")
...
df.to_csv('space_testConfidence2.csv', index=False)
...
for idx in range(0, len(df)):
    ...
    try:
        ...
        if not available_clients:
            client = client1
            print("⚠️ All secondary APIs exhausted. Using client1 only.")
        else:
...
"""
        ...
        try:
            ...
            if "quota" in str(e).lower() or "limit" in str(e).lower():
                ...
                print(f"❌ API limit reached for {name}, removing from rotation.")
                client = client1
                content = query_model(prompt, client, retries=1)
            else:
                raise
...
# === LOAD DATA ===
df = pd.read_excel("/content/complete_noBlanks.xlsx")
df['description'] = df['description'].fillna('')
...
"""
        ...
        # Save progress after error
        df.loc[idx, 'raw_resp'] = err_output
        df.to_csv('/content_2025_v07.csv', index=False)
...
def query_model(prompt, client, model="meta-llama/llama-3.3-70b-instruct:free", retries=1):
    for attempt in range(retries + 1):
        try:
            print(f"\n🚀 Sending prompt (length={len(prompt)} chars) to model: {model} ...")
            response = client.chat.completions.create(
                model=model,
                messages=[
...
# === LOAD DATA ===
df = pd.read_excel("/content/space_189.xlsx")
df['description'] = df['description'].fillna('')
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if not hasattr(response, 'choices') or not response.choices:
                print("⚠️ Warning: No choices found in response.")
...
print(f"• Technical: {count_technical}")
print(f"• Non-technical: {count_non_technical}")
